...
const TrackList: React.FC = () => {
    ...
    return (
      <div className="px-4 md:px-6 py-1">
        <div
          ...
          className={`
            group flex items-center h-[64px] md:h-[50px] rounded-xl cursor-pointer transition-colors border-b border-black/5 dark:border-white/5 md:border-none
            ${isCurrent ? 'bg-black/5 dark:bg-white/10' : 'active:bg-black/5 dark:active:bg-white/5 md:hover:bg-black/5 dark:md:hover:bg-white/5'}
          `}
        >
          {/* COL 1: INDEX / INITIALS */}
          <div className="w-10 flex items-center justify-center shrink-0">
            <div className="md:hidden w-9 h-9 bg-zinc-200 dark:bg-zinc-800 rounded-md flex items-center justify-center text-zinc-600 dark:text-zinc-500 font-bold text-[10px] overflow-hidden">
          ...
          <div className="flex-1 min-w-0 px-3 md:px-4 flex flex-col justify-center">
            <div className={`text-[15px] md:text-[14px] font-medium truncate ${isCurrent ? 'text-[#fa2d48]' : 'text-zinc-900 dark:text-zinc-100'}`}>
              {track.title}
            </div>
  ...
  if (isLoading) return <div className="flex-1 flex items-center justify-center text-zinc-500 bg-white dark:bg-[#1c1c1e] h-full transition-colors duration-300">Loading...</div>;
  if (!filteredTracks.length) return <div className="flex-1 flex flex-col items-center justify-center text-zinc-500 h-full bg-white dark:bg-[#1c1c1e] pb-32 transition-colors duration-300"><p className="mb-2">Library Empty</p></div>;

  return (
    // PERUBAHAN 1: Hapus 'pb-48 md:pb-40' dari sini. Biarkan container full height.
    <div className="flex-1 flex flex-col bg-white dark:bg-[#1c1c1e] min-h-0 transition-colors duration-300">

      <div className="flex items-center px-4 md:px-6 h-10 border-b border-black/5 dark:border-white/5 bg-white/95 dark:bg-[#1c1c1e]/95 backdrop-blur-md sticky top-0 z-10 shrink-0 transition-colors duration-300">
