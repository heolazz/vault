    ...
    <>
      ...
      <div className="md:hidden fixed left-2 right-2 bottom-[90px] z-[100]">
        <div 
            ...
            className="
                bg-[#252525]/95 backdrop-blur-xl border border-white/10 
        ...
        >
            ...
            <div className="w-12 h-12 rounded-xl bg-zinc-800 shrink-0 overflow-hidden relative shadow-md ml-1 z-10 flex items-center justify-center">
                <div className="text-zinc-500 font-bold text-lg">{currentTrack.title.charAt(0)}</div>
            </div>

            <div className="flex-1 min-w-0 px-3 flex flex-col justify-center z-10">
                <span className="text-white font-semibold text-[14px] truncate leading-tight">{currentTrack.title}</span>
                <span className="text-zinc-400 text-xs truncate mt-0.5">{currentTrack.artist}</span>
            </div>

            <div className="flex items-center gap-3 z-10">
                <button onClick={(e) => { e.stopPropagation(); toggleFavorite(currentTrack); }} className={`${currentTrack.isFavorite ? 'text-[#fa2d48]' : 'text-zinc-500'} active:scale-90 transition-transform`}>
                    <Heart className={`w-5 h-5 ${currentTrack.isFavorite ? 'fill-current' : ''}`} />
                </button>
                <button onClick={(e) => { e.stopPropagation(); togglePlay(); }} className="text-white w-10 h-10 flex items-center justify-center active:scale-90 transition-transform">
                    {isPlaying ? <Pause className="w-6 h-6 fill-white" /> : <Play className="w-6 h-6 fill-white ml-0.5" />}
                </button>
                <button onClick={(e) => { e.stopPropagation(); nextTrack(); }} className="text-zinc-400 hover:text-white active:scale-90 transition-transform">
